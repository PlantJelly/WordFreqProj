...
def tokenize_korean_corpus(corpus, tokenizer, my_tags=None, my_stopwords=None):
    ...
    if my_tags and my_stopwords:
        ...
        for text in corpus:
            tokens = [word for word, tag in tokenizer(text) if tag in my_tags]
            all_tokens += tokens
    elif my_stopwords:
        for text in corpus:
        ...
        for text in corpus:
            tokens = [word for word, tag in tokenizer(text)]
            all_tokens += tokens
    return all_tokens
...
def visuaslize_barhgraph(counter, num_words, title, xlabel, ylabel, font_path=None):
...
def visuaslize_wordcloud(counter, num_words, font_path=None):
